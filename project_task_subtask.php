...
<?php
            ...
            if (isset($_POST['mark_complete'])){
                $task_id=intval($_POST['subtask_id']);
		...
		<div id="main">
           ...
           <?php include ("include/menu.php"); ?>
               ...
               <?php
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    //echo "<div id='project_short_details_wrap'>";
						echo "<span class='project_name'>$project_name<br></span>";   //boldovo
			...
			<?php
                            ...
                            $sql="SELECT * from project_task_subtasks WHERE task_id=$subtask_id";
                            //echo "$sql"; 
                            $result=mysqli_query($db, $sql) or die("MySQL ERROR: ".mysqli_error());
                            while ($row = mysqli_fetch_array($result)) {
              ...
              <div id="project_task_details_dashboard"><!-- project_task_details_dashboard -->
					...
					<h3>Dashboard</h3>
						...
						<div class="info_box">
                            ...
                            <span class="info_box_value"><?php  
                                if($duration==0){
                                    echo CurrSubTaskDuration($subtask_id);
                                } else {
                                    echo $duration;
                                }
						...
						<div class="info_box">
							<span class="info_box_title">Mark as complete:</span>
                            <span class="info_box_value"><?php 
                                    if($is_complete==1){
                                        echo "</span>";
                                    } else {
                            echo "<form action='' method='post'><input type='hidden' name='subtask_id' value='$subtask_id'><button name='mark_as_complete' type='submit' class='blue-badge'><i class='fa fa-check'></i></button></form></span>";} ?>
