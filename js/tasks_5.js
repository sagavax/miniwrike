...
tasks.addEventListener("click", function(ev) {
    ...
    if (taskElement) {
        ...
        } else if (ev.target.classList.contains("task_updates") || ev.target.innerHTML==="<i class='fas fa-plus-circle'></i>"){
            localStorage.setItem("task_id", taskId);
            //alert("task updates task: " + taskId);
...
function GetNumberofUpdates(taskId){
     ...
     xhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            document.querySelector(".task_updates").innerHTML = xhttp.responseText
        }
