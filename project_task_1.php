...
if (isset($_POST['add_task_subtask'])) { //pridanie subtasku
    ...
    $sql1 = "INSERT INTO project_task_subtasks (task_id, project_id, user_id, subtask_description,subtaskstatus, subtask_priority, subtask_created, subtask_finished, subtask_deadline) VALUES ($task_id,$project_id, $user_id,'$subtask_name', 'new','normal','$date_added','','$subtask_deadline')";
...
if($_POST['unassign_task']){
	$task_id=intval($_POST['task_id']);
	$user_id=intval($_POST['user_id']);
	$date_added = date('Y-m-d H:m:s');
    $assigment_id=intval($POST['assigment_id']);
...
if (isset($_POST['edit_task_details'])) { //kliknem na update button
    ...
    $sql = "UPDATE project_tasks SET task_finished='$date_finished',status='$task_status',task_priority='$task_priority',task_deadline='$task_deadline' WHERE task_id=$task_id";
				   ...
				   <div id="info_box_wrap" style="width:100%;float:left; min-height:250px">
				      <form action="project_task.php" method="POST">
				         ...
				         <div class="info_box">
				            ...
				            <span class="info_box_value"><input type='text' name='task_deadline' value="<?php echo $deadline ?>"></span>	
				         </div>
				         <div class="info_box">
				...
				<div id="project_task_subtasks"> <!-- vsetky taskove komentare  + moznost pridat novy komentar-->
					...
					<div id="task_subtasks_wrap"> <!--subtask wrap -->
						  ...
						  <?php
							...
							if ($numrows==0) {echo "<span style='font-style:italic; font-size:12px; font-family:'Roboto', Helvetica, Arial,sans-serif;color:#ddd;margin-left:10px; margin-top:10px'>No subtasks available</span>";} else {
								...
								while ($row = mysql_fetch_array($result)) {
									...
									$dStart = strtotime($subtask_created);
									$dEnd = strtotime(date('Y-m-d'));
									$dDiff = $dEnd - $dStart;
									$duration=date('j',$dDiff);
									...
									echo "<li>"; //project_task_comments
										echo "<div class='project_subtask_post'>
												...
												<div class='subtask_priority'>$priority</div>
												<div class='subtask_date'>$task_created</div>
												<div class='subtask_status'><span style='border:1px #666 solid;padding:5px;border-radius:5px'>$status</span></div>
						...
						<table>
								...
								<tr>
									<td><input name="subtask_name" type="text" value=""><button type="submit" class="blue-badge" name="add_task_subtask" alt="Add subtask"><i class="fa fa-plus"></i></button></td> <!-- subtask button -->
								</tr>
							</form>
						</table>
							...
							<?php
                                ...
                                if ($numrows==0) {echo "<span style='font-style:italic; font-size:12px; font-family: Helvetica, Arial,sans-serif;color:#ccc;margin-left:10px; margin-top:10px'>No ppl assigned to this task</span>";} else { //ak existuju nejake poznamky tak ich vypis
                                    ...
                                    while ($row = mysql_fetch_array($result)) {
                                        ...
                                        $image="images/non-avatar_40.jpg";
										$duration=$row['duration'];
